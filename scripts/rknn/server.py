...
def rotated_nms(boxes, scores, iou_thres=0.45, max_det=100):
    ...
    # 快速路径：候选少时直接取 top-K，跳过昂贵的 IoU 计算
    if len(order) <= 15:
...
def postprocess_obb(output, img_shape, conf_thres=0.25):
    ...
    keep = rotated_nms(boxes, conf, IOU_THRES)
...
def preprocess(img):
    ...
    inp = cv2.cvtColor(canvas, cv2.COLOR_BGR2RGB)  # UINT8 [0,255]
...
def inference_loop(rknn):
    ...
    while state.running:
        ...
        time.sleep(0.005)  # 极短 sleep 让出 CPU
